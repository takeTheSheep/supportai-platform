  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="card-elevated rounded-[1.75rem] border border-border/70 p-8 sm:p-9">
        ...
        <h1 className="text-3xl font-semibold text-heading">Create your account</h1>
        <p className="mt-2 text-sm text-muted">
          Start a new workspace with demo-ready support permissions and seeded product flows.
        </p>
        ...
        <form className="mt-6 space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
          <div>
            ...
            {form.formState.errors.name ? (
              <p className="mt-1.5 text-xs text-rose">{form.formState.errors.name.message}</p>
            ) : null}
          </div>
          ...
          {error ? (
            ...
            <p className="rounded-xl bg-tealSoft px-3 py-2 text-sm text-teal">
              Provisioned with support agent permissions by default.
            </p>
          )}
        ...
        <p className="mt-6 text-sm text-muted">
          Already have access?{" "}
          <Link href="/login" className="font-medium text-primary transition duration-200 hover:text-primaryDeep">
            Sign in
          </Link>
        </p>
      </div>
